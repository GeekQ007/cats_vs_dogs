# -*- coding: utf-8 -*-
"""
Created on Mon Dec 16 00:19:42 2019
            ...
            for epoch in range(pre_epoch, total_epoch):
                ...
                with torch.no_grad():
                    ...
                    torch.save(use_model.state_dict(),
                               '%s/net_%3d.pth' % (args.outf, epoch + 1))
                    acc_f.write("epoch = %03d, accuracy = %.3f%%" %
                                (epoch + 1, acc))
                    acc_f.write('\n')
                    acc_f.flush()
                    # 记录最佳的测试准确率
                    if acc > best_test_acc:
                        ...
                        best_test_acc = acc
                        # 写入tensorboard
                        writer.add_scalar('best_accuracy/test',
                                          best_test_acc, epoch)
